...
export async function GET(req) {
    ...
    const fetchMembers = async (token) => {
        const url = `${process.env.NEXT_PUBLIC_API_BASE_URL}/${shortName.toLowerCase()}/${uniqueId}/api/v2/members/getAll`;
        return fetch(url, {
            headers: {
    ...
    try {
        let response = await fetchMembers(accessToken);
        ...
        if (!response.ok && response.status === 401) {
            ...
            if (refreshToken) {
                ...
                if (tokenRes.ok) {
                    ...
                    const retryRes = await fetchMembers(token.access_token);
                    if (retryRes.ok) {
