...
async function handleRequest(req, params, method) {
    ...
    const fetchCash = async (token) => {
        const url = new URL(`${process.env.NEXT_PUBLIC_API_BASE_URL}/${shortName.toLowerCase()}/${uniqueId}/api/v2/cash/${actionPath}`);
    ...
    try {
        let response = await fetchCash(accessToken);
            ...
            if (refreshToken) {
                try {
                    ...
                    if (tokenRes.ok) {
                        ...
                        const retryRes = await fetchCash(token.access_token);
                    ...
                    console.error('Cash proxy refresh failed:', refreshErr);
                }
            }
        }
        ...
        if (!response.ok) {
            ...
            return NextResponse.json({
                ok: false,
                message: errorResult.message || `Failed to ${actionPath || method} cash data.`
        ...
        console.error(`Cash Proxy ${actionPath || method} Error:`, error);
