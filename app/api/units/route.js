...
export async function GET(req) {
    ...
    const fetchUnits = async (token) => {
        const url = `${process.env.NEXT_PUBLIC_API_BASE_URL}/${shortName.toLowerCase()}/${uniqueId}/api/v2/units/getAll`;
        return fetch(url, {
            headers: {
    ...
    try {
        let response = await fetchUnits(accessToken);
        ...
        if (!response.ok && response.status === 401) {
            ...
            if (refreshToken) {
                ...
                if (tokenRes.ok) {
                    ...
                    const retryRes = await fetchUnits(token.access_token);
                    if (retryRes.ok) {
