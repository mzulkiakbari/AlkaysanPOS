...
export default function CashFlowReportPage() {
    ...
    const [summary, setSummary] = useState({ total: 'Rp 0' });
    ...
    const fetchReport = async () => {
        ...
        try {
            ...
            const result = await FetchData({
                method: 'GET',
                uri: `/api/reports/cash-flow?${params.toString()}`
            });

            if (result) {
                setReportData(result.kas || []);
                setSummary({ total: result.total || 'Rp 0' });
                ...
                <div className="flex flex-row items-start justify-between bg-white md:p-8 rounded-[2.5rem] border border-gray-100 md:shadow-sm relative overflow-hidden print:border-none print:shadow-none print:p-0 print:mb-6 print:bg-transparent print:rounded-none">
                    <div className="flex flex-row items-center gap-6">
                        ...
                        <div className="space-y-1 z-10">
                            <h1 className="text-xl md:text-3xl font-black text-gray-900 uppercase tracking-tight">LAPORAN ARUS KAS</h1>
                            <h2 className="text-md md:text-xl font-bold text-primary uppercase">{selectedBranch?.storeName || 'BRANCH NAME'}</h2>
                            <p className="text-[10px] md:text-xs text-gray-500 font-medium max-w-md leading-relaxed">{selectedBranch?.storeData?.address || 'Branch Address'}</p>
                        </div>
                    </div>
                    ...
                    <div className="text-right space-y-4 z-10">
                        ...
                        <div className="flex gap-4 justify-end print:justify-start">
                            <div className="text-right print:text-left">
                                <p className="text-[10px] font-black text-primary uppercase tracking-widest print:text-black">Total</p>
                                <p className="text-sm md:text-lg font-black text-gray-900 tabular-nums print:text-black print:text-sm">{summary.total}</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table className="w-full text-left print:text-[7pt]">
                            <thead>
                                <tr className="bg-gray-50/50 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 print:bg-gray-100 print:text-black print:border-black">
                                    <th className="px-6 py-6 text-center print:px-2 print:py-2">No</th>
                                    <th className="px-6 py-6 whitespace-nowrap print:px-2 print:py-2">Tanggal</th>
                                    <th className="px-6 py-6 print:px-2 print:py-2">Kas</th>
                                    <th className="px-6 py-6 print:px-2 print:py-2">No Trans</th>
                                    <th className="px-6 py-6 print:px-2 print:py-2">Pemesan</th>
                                    <th className="px-6 py-6 print:px-2 print:py-2">Keterangan</th>
                                    <th className="px-6 py-6 text-right print:px-2 print:py-2">Masuk</th>
                                    <th className="px-6 py-6 text-right print:px-2 print:py-2">Keluar</th>
                                    <th className="px-6 py-6 print:px-2 print:py-2">Oleh</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50 print:divide-y print:divide-black">
                                {isLoading ? (
                                    <tr>
                                        <td colSpan="9" className="py-20 text-center text-gray-400 italic font-bold">
                                            <ArrowPathIcon className="w-10 h-10 animate-spin mx-auto text-primary/20" />
                                        </td>
                                    </tr>
                                ) : reportData && reportData.length > 0 ? (
                                    reportData.map((item, idx) => (
                                        <tr key={idx} className="hover:bg-gray-50/50 transition-colors text-[10px] md:text-xs print:hover:bg-transparent">
                                            <td className="px-6 py-4 text-center text-gray-400 font-bold print:px-2 print:py-1 print:text-black print:font-normal">{idx + 1}</td>
                                            <td className="px-6 py-4 text-gray-600 font-medium whitespace-nowrap print:px-2 print:py-1 print:text-black">{item.tanggal}</td>
                                            <td className="px-6 py-4 font-bold text-gray-500 uppercase print:px-2 print:py-1 print:text-black print:font-normal">{item.kas || '-'}</td>
                                            <td className="px-6 py-4 font-medium text-gray-400 uppercase print:px-2 print:py-1 print:text-black print:font-normal">{item.no_transaksi || '-'}</td>
                                            <td className="px-6 py-4 font-black text-gray-900 uppercase print:px-2 print:py-1 print:text-black print:font-normal">{item.nama_pemesan || '-'}</td>
                                            <td className="px-6 py-4 text-gray-500 italic leading-tight max-w-[150px] print:px-2 print:py-1 print:text-black print:font-normal print:not-italic">{item.keterangan || '-'}</td>
                                            <td className="px-6 py-4 text-right font-black text-emerald-600 print:px-2 print:py-1 print:text-black print:font-normal">
                                                {item.masuk}
                                            </td>
                                            <td className="px-6 py-4 text-right font-black text-red-600 print:px-2 print:py-1 print:text-black print:font-normal">
                                                {item.keluar}
                                            </td>
                                            <td className="px-6 py-4 font-bold text-gray-700 uppercase italic text-[9px] print:px-2 print:py-1 print:text-black print:font-normal print:not-italic">{item.input_by || '-'}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                                        <td colSpan="9" className="py-20 text-center text-gray-400 italic font-bold uppercase tracking-widest">Data Tidak Tersedia</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
