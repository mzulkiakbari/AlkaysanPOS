    ...
    const fetchReport = async () => {
        ...
        try {
            ...
            const result = await FetchData({
                method: 'GET',
                uri: `/api/reports/sales?${params.toString()}`
            });

            if (result && result.success === undefined) { // Check if it's the new direct response structure
                setReportData(result.penjualan || []);
                setTotalSales(result.total || 'Rp 0');
                setPeriode(result.periode || '-');
            } else if (result && result.success) { // Fallback/compatibility
                setReportData(result.data.penjualan || result.data.data || []);
                setTotalSales(result.data.total || 'Rp 0');
        ...
        <MainLayout>
            ...
            <style jsx global>{`
                @media print {
                    ...
                    body { background: white; padding: 0.5cm; margin: 0; color: black; font-size: 9pt; }
                    .main-content { padding: 0 !important; margin: 0 !important; }
                    
                    /* Classic Table Styles */
                    table { border-collapse: collapse !important; width: 100% !important; margin-top: 10px; }
                    th, td { border: 1px solid #000 !important; padding: 4px 8px !important; text-align: left !important; color: black !important; }
            ...
            <div className="space-y-8 animate-fade-in relative px-4 md:px-0">
                ...
                <div className="flex flex-row items-start justify-between bg-white md:p-8 rounded-[2.5rem] border border-gray-100 md:shadow-sm relative overflow-hidden print:border-none print:shadow-none print:p-0 print:mb-8 print:bg-transparent print:rounded-none">
                    <div className="flex flex-row items-center gap-6">
                        ...
                        <div className="space-y-1 z-10">
                            <h1 className="text-xl md:text-3xl font-black text-gray-900 uppercase tracking-tight">LAPORAN PENJUALAN</h1>
                    ...
                    <div className="text-right space-y-4 z-10">
                        <div className="space-y-1">
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Periode</p>
                            <div className="flex items-center justify-end gap-2 text-gray-700 font-bold text-xs md:text-sm">
                                <CalendarIcon className="w-4 h-4 text-primary no-print" />
                                <span>{periode}</span>
                            </div>
                        </div>
                        <div className="flex flex-col items-end print:items-start">
                            <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest print:text-black">Total Penjualan</p>
                            <p className="text-xl md:text-3xl font-black text-emerald-600 tracking-tighter tabular-nums print:text-black print:text-xl">{totalSales}</p>
                        </div>
                    </div>
                </div>

                {/* Table Section */}
                <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden min-h-[400px] print:border-none print:shadow-none print:rounded-none print:p-0">
                    <div className="overflow-x-auto print:overflow-visible">
                        <table className="w-full text-left print:text-[8pt]">
                            <thead>
                                <tr className="bg-gray-50/50 text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 print:bg-gray-100 print:text-black print:border-black">
                                    <th className="px-8 py-6 print:px-2 print:py-2">No Transaksi</th>
                                    <th className="px-8 py-6 print:px-2 print:py-2">Tanggal</th>
                                    <th className="px-8 py-6 print:px-2 print:py-2">Pemesan</th>
                                    <th className="px-8 py-6 print:px-2 print:py-2">Produk</th>
                                    <th className="px-8 py-6 text-center print:px-2 print:py-2">Qty</th>
                                    <th className="px-8 py-6 print:px-2 print:py-2">Member</th>
                                    <th className="px-8 py-6 print:px-2 print:py-2">CS</th>
                                    <th className="px-8 py-6 text-right print:px-2 print:py-2">Subtotal</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-50 print:divide-y print:divide-black">
                                {isLoading ? (
                                    <tr>
                                        <td colSpan="8" className="py-20 text-center">
                                            <ArrowPathIcon className="w-10 h-10 animate-spin mx-auto text-primary/20" />
                                            <p className="mt-4 text-sm font-bold text-gray-400 uppercase tracking-widest">Memuat Data...</p>
                                        </td>
                                    </tr>
                                ) : reportData && reportData.length > 0 ? (
                                    reportData.map((item, idx) => (
                                        <tr key={idx} className="hover:bg-gray-50/50 transition-colors group print:hover:bg-transparent">
                                            <td className="px-8 py-5 font-bold text-gray-500 group-hover:text-primary transition-colors uppercase tracking-tight print:px-2 print:py-1 print:text-black print:font-normal">
                                                {item.no_transaksi}
                                            </td>
                                            <td className="px-8 py-5 text-gray-600 font-medium whitespace-nowrap text-xs print:px-2 print:py-1 print:text-black">
                                                {item.tanggal}
                                            </td>
                                            <td className="px-8 py-5 font-black text-gray-900 uppercase text-xs print:px-2 print:py-1 print:text-black print:font-normal">
                                                {item.pemesan || '-'}
                                            </td>
                                            <td className="px-8 py-5 text-xs font-bold text-gray-700 max-w-[200px] leading-relaxed print:px-2 print:py-1 print:text-black print:font-normal">
                                                {item.nama_produk || '-'}
                                            </td>
                                            <td className="px-8 py-5 text-center print:px-2 print:py-1">
                                                <div className="flex flex-col items-center">
                                                    <span className="text-sm font-black text-gray-900 print:text-[8pt] print:font-normal">{item.qty || 0}</span>
                                                    <span className="text-[8px] font-black text-gray-400 uppercase no-print">{item.total_item || 0} Item</span>
                                                </div>
                                            </td>
                                            <td className="px-8 py-5 print:px-2 print:py-1">
                                                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider print:p-0 print:font-normal ${item.membership === 'STUDIO' ? 'bg-indigo-50 text-indigo-500 print:bg-transparent' : 'bg-gray-100 text-gray-500 print:bg-transparent'
                                                    }`}>
                                                    {item.membership || 'UMUM'}
                                                </span>
                                            </td>
                                            <td className="px-8 py-5 font-bold text-gray-700 uppercase italic text-[9px] max-w-[120px] print:px-2 print:py-1 print:text-black print:font-normal print:not-italic">
                                                {item.cs || '-'}
                                            </td>
                                            <td className="px-8 py-5 text-right font-black text-gray-900 text-base tracking-tighter print:px-2 print:py-1 print:text-black print:font-normal print:text-[8pt]">
                                                {item.subtotal}
                                            </td>
                                        </tr>
                                    ))
